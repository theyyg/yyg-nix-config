...
** MS Windows-specific Config
This configuration only applies if emacs is run in a Microsoft Windows environment.

#+begin_src emacs-lisp
;  (when (string= system-type "windows-nt")
;    (progn
;      (setq diff-path "path/to/diff")
;      ))
#+end_src
...
#+RESULTS:
:  Successfully installed ‘nerd-icons’ fonts to ‘~/.local/share/fonts/’!
...
** Stripe Buffer
Alternates the color of every other line.  I use this in org tables.
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
  ...
  (defun efs/org-font-setup ()
    ...
    (set-face-attribute 'org-column nil :background nil :inherit '(shadow))
...
#+begin_src emacs-lisp
  ...
  (use-package org
    ...
    (setq org-agenda-files
          '("~/dev/notes/tasks.org"
            "~/dev/notes/todo.org"
            "~/dev/notes/journal.org"
            "~/dev/notes/orontes.org"
            "~/dev/notes/ofire.org"))
    ...
    (setq org-todo-keywords
          '((sequence "TODO(t)" "|" "DONE(d!)")
                                          ...
                                          ; Bugs
            (sequence "TRIAGE(T)" "DIAGNOSE(D)" "IN WORK(I)" "WAIT(W)" "|" "FIXED(F!)" "VERIFY(V)" "CLOSED(C!)")
                                          ; Tasks
            (sequence "PROPOSAL(z)" "BACKLOG(b)" "ON DECK(o)" "PLAN(p)" "IN PROGRESS(i)" "ON HOLD(h)" "IMPLEMENTED(m)" "REVIEW(v)" "INTEGRATED(g)" "|" "POSTPONED(w@)" "REJECTED(j!)" "COMPLETED(c!)" "CANCELED(k@)"))
                                          ;  
          ;;(sequence "BACKLOG(b)" "ASSIGNED(a)" "HOLD(h)" "ON DECK(o)" "PLAN(p)" "IN PROGRESS(i)" "IMPLEMENTED(m)" "INTEGRATED(g)" "REVIEW(v)" "|" "POSTPONED(w!)" "REJECTED(j!)" "COMPLETED(c!)" "CANCELED(k@)"))
          )

    (setq org-refile-targets
          '(("task-archive.org" :maxlevel . 1)
    ...
    (setq org-columns-default-format
          "%3PRIORITY %DEADLINE %TODO %TAGS %50ITEM")

    ;; Configure custom agenda views
    (setq org-agenda-custom-commands
          '(("d" "Dashboard"
             ((agenda "" ((org-deadline-warning-days 100)))
              (todo "IN PROGRESS|IN WORK" ((org-agenda-overriding-header "Active Projects")))
              (todo "ON DECK|TRIAGE|DIAGNOSE" ((org-agenda-overriding-header "Next Tasks")))))

            ("D" "Deadlines"
             ((tags "DEADLINE>=\"<today>\"&DEADLINE<\"<+99d>\""
                    ((org-agenda-entry-types '(:deadline))
                     (org-agenda-overriding-header "This Season")
                     (org-agenda-sorting-strategy '(priority-down))))
              (tags "DEADLINE>=\"<+99d>\"&DEADLINE<\"<+189d>\""
                    ((org-agenda-entry-types '(:deadline))
                     (org-agenda-overriding-header "Next Season")
                     (org-agenda-sorting-strategy '(priority-down)))))
             nil
             ("~/dev/notes/exported/deadline_next_season.txt"))
            ...
            ("R" "Season Review"
             ((agenda ""
                      ((org-agenda-show-all-dates t)
                       ...
                       (org-agenda-span 90) ;; (org-agenda-span 'week)
                       (org-agenda-start-on-weekday 0)
                       (org-agenda-overriding-header "Season in Review")))))
            ...
            ("r" "Weekly Review"
             ((agenda "+journal"
                      ((org-agenda-show-all-dates t)
                       ...
                       (org-agenda-span 14) ;; (org-agenda-span 'week)
                       (org-agenda-start-on-weekday 0)
                       (org-agenda-overriding-header "Week in Review")))))


            ("n" "Next Tasks"
             ((todo "ON DECK|PLAN|DIAGNOSE|TRIAGE"
                    ((org-agenda-overriding-header "Next Tasks")))))

            ("G" "Aussie Greg" tags-todo "+aussie")
            ...
            ("x" "Low Effort Tasks" tags-todo "+TODO=\"ON DECK|TRIAGE|DIAGNOSE\"+Effort<15&+Effort>0"
             ...
             ((todo "WAIT|ON HOLD"
              ...
              (todo "REVIEW"
                    ((org-agenda-overriding-header "In Review")
                     (org-agenda-files org-agenda-files)))
              (todo "IMPLEMENTED"
                    ((org-agenda-overriding-header "Local Testing")
                     (org-agenda-files org-agenda-files)))
              (todo "IN PROGRESS|IN WORK"
                    ((org-agenda-overriding-header "In Progress")
                     (org-agenda-files org-agenda-files)))
              (todo "PLAN|ON DECK|DIAGNOSE"
                    ((org-agenda-overriding-header "In Preparation")
              ...
              (todo "BACKLOG|TODO"
                    ((org-agenda-overriding-header "Project Backlog")
                     (org-agenda-todo-list-sublevels nil)
                     (org-agenda-files org-agenda-files)))
              (todo "CLOSED|COMPLETED"
                    ((org-agenda-overriding-header "Completed Projects")
                     (org-agenda-files org-agenda-files)))
              (todo "POSTPONED"
                    ((org-agenda-overriding-header "Postponed Projects")
                     (org-agenda-files org-agenda-files)))
    ...
    (setq org-capture-templates
          `(("t" "Tasks / Projects")
            ("tt" "Task" entry (file+olp "~/dev/notes/tasks.org" "Inbox")
             "* TODO %?\n  %U\n  %a\n  %i" :empty-lines 1)
            ("ts" "Clocked Entry Substask" entry (clock)
             "* TODO %?\n %U\n %a\n %i" :empty-lines 1)

            ("j" "Journal Entries")
            ("jj" "Journal" entry
             (file+olp+datetree "~/dev/notes/journal.org")
            ...
            ("w" "Workflows")
            ("we" "Checking Email" entry (file+olp+datetree "~/dev/notes/journal.org")
             "* Checking Email :email:\n\n%?" :clock-in :clock-resume :empty-lines 1)

            ("m" "Metrics Capture")
            ("mw" "Weight" table-line (file+headline "~/dev/notes/metrics.org" "Weight")
...
Using LaTeX in org-mode can be done by including the following org-headers:

**** LaTeX Setup File
Creating well-formatted LaTeX documents using the org-mode structure can require a lot of LaTeX-specific headers and customization.  Including this SETUPFILE gives me a consistent and simple configuration for use across org files.

To use this configuration, include a SETUPFILE org header instead of LATEX_CLASS, LATEX_CLASS_OPTIONS, and OPTIONS.

#+begin_src org :notangle :tangle no
  #+SETUPFILE: latex-standard.setup
#+end_src

This section contains the contents of the SETUPFILE for LaTeX specific org-babel processing.  This configuration was taken from and inspired from https://jakebox.github.io/youtube/org_latex_video.html 

Because this include org-specific code, edit this using ~C-c C-'~.

#+begin_src text :tangle ./.emacs.d/latex-standard.setup
...
[[https://github.com/nobiot/org-transclusion?tab=readme-ov-file][org-transclusion]] is a dynamic and powerful inclusion framework that allows multiple files to share the same content without multiple copies.
...
#+begin_src emacs-lisp
  ...
  :config
    ...
    (define-key org-mode-map (kbd "C-<f12>") #'org-transclusion-mode)
